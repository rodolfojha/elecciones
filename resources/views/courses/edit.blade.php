    ...
    <div class="py-6">
        <div class="mx-auto sm:px-6 lg:px-8 max-w-3xl">
            <div class="mb-6">
                ...
                <p class="text-gray-600 dark:text-gray-400">Actualiza la información del curso</p>
            </div>

            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow rounded-lg border border-gray-200 dark:border-gray-700">
                <form action="{{ route('courses.update', $course) }}" method="POST" enctype="multipart/form-data" class="px-4 py-5 sm:p-6">
                    @csrf
                    @method('PUT')
                    ...
                    @if($course->thumbnail)
                        <div class="mb-4">
                            ...
                            <img src="{{ Storage::url($course->thumbnail) }}" alt="Thumbnail" class="w-32 h-32 object-cover rounded-md">
                        </div>
                    @endif
                    ...
                    <div class="mb-6">
                        <label for="thumbnail" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            {{ $course->thumbnail ? 'Cambiar Imagen de Portada' : 'Imagen de Portada' }}
                        </label>
                        <input 
                            type="file" 
                            name="thumbnail" 
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div>
                            ...
                            <select 
                                ...
                                required
                            >
                                <option value="Inglés" {{ old('tipo', $course->tipo) === 'Inglés' ? 'selected' : '' }}>Inglés</option>
                                <option value="Programas técnicos laborales" {{ old('tipo', $course->tipo) === 'Programas técnicos laborales' ? 'selected' : '' }}>Programas técnicos laborales</option>
                            </select>
                            @error('tipo')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="flex items-center justify-end space-x-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                        <a href="{{ route('courses.show', $course) }}" class="inline-flex items-center px-4 py-2 bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 text-sm font-medium rounded-md transition-colors">
